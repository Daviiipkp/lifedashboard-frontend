...
import { Home, PlusCircle, Sun, Moon, User, BarChart2, Target, Menu, X} from 'lucide-react';
import { useLocation } from "react-router-dom";
import type React from "react";
import { use, useEffect, useState } from "react";

function getRedirectionWidget(name: string, icon: React.ReactNode, mobileNavOpen?: boolean, setMobileNavOpen?: React.Dispatch<React.SetStateAction<boolean>>) {
  const location = useLocation();
  const isActive = (path: string) => location.pathname === path;
  if(name === "navBarOpener" && setMobileNavOpen && mobileNavOpen !== undefined) {
              ...
              !isMobile ? (
                ...
                mobileNavOpen ? (
                  <div className="relative top-30 z-50 flex flex-col items-center gap-2 bg-black/90 backdrop-blur-xl border border-white/10 p-3 rounded-2xl shadow-xl min-w-[60px]">
                    ...
                    <div onClick={() => setMobileNavOpen(false)} className="cursor-pointer text-red-400">
                        <X size={20} />
                    </div>

                  </div>
                ) : (getRedirectionWidget("navBarOpener", <Menu size={24} />, mobileNavOpen, setMobileNavOpen)
                )
              )
            ) : (
            <>

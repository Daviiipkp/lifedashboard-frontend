...
export function Register() {
  ...
  function checkForm(toCheck: string, ): void {
    ...
    if (username.length < 4 && (toCheck === "username" || submit) && username.length > 0) {
    ...
    if ((!email.includes("@") || !email.includes(".") || email.includes(" ")) && (toCheck === "email" || submit) && email.length > 0) {
        return "Please enter a valid email address.";
    }

    if (password.length < 8 && (toCheck === "password" || submit) && password.length > 0) {
        return "Password must be at least 8 characters.";
    }

    if (password != confirmPassword && ((((toCheck === "password" || toCheck === "confirmPassword" )) && (password.length > 0 && confirmPassword.length > 0)) || submit)) {
    ...
    return (
        <div className="w-full h-screen flex flex-col items-center justify-center">
          <div className="w-[90%] max-w-sm md:max-w-md lg:max-w-md p-5 bg-gray-700 flex-col flex items-center gap-3 rounded-xl shadow-2xl shadow-blue-500/20">
                <TextLogo />
                <p className="text-xs md:text-sm text-gray-500 text-center select-none">Already have an account? <span className="text-white cursor-pointer select-none" onClick={() => navigate("/login")}>Login</span></p>
            ...
            <div className="w-full flex flex-col gap-3">
              ...
              <div className="w-full flex items-center bg-gray-900 p-2 rounded-2xl gap-3 relative">
                ...
                /> 
                {showPassword ?
                (<BsFillEyeFill onClick={() => setShowPassword(!showPassword)} className="absolute right-4 cursor-pointer"/> )
                  : (<BsEyeSlashFill onClick={() => setShowPassword(!showPassword)} className="absolute right-4 cursor-pointer"/>)}
               
              </div>
  
            </div>
  
            <button className="w-full bg-amber-500 md:text-base hover:bg-amber-400 text-black font-semibold py-2 rounded-2xl
            transition-colors" onClick={handleSubmit}>
              Sign Up
            </button>

            <div>
              {errorMsg && (
                <p className="text-red-500 text-sm text-center select-none">{errorMsg}</p>
              )}
            </div>
